...
def attached(func):
    """
    this is a decorator to use to wrap functions that require
    a working connection to the telescope.
    """
    def wrapped(self=None, *args, **kwargs):
        if self is not None:
            ...
            self.log.warning("Must supply NexStar() instance.")
            return False
    return wrapped
...
def aligned(func):
    """
    this is a decorator to use to wrap functions that are only
    safe to run after an alignment has been completed.
    """
    def wrapped(self=None, *args, **kwargs):
        if self is not None:
            if self.isAligned():
                ...
                self.log.error("Command unsafe to run when telescope is not aligned.")
                return False
        else:
            self.log.warning("Must supply NexStar() instance.")
            return False
...
class NexStar:
    ...
    def nudge(self, direction, rate):
        """
        macro to do a move in a direction for .3 sec and then stop without
